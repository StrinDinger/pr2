...
export function StockPickerDialog({ open, onOpenChange, onSelectStock }: StockPickerDialogProps) {
  ...
  const fetchStocks = async () => {
    ...
    try {
      ...
      const response = await fetch('https://iss.moex.com/iss/engines/stock/markets/shares/boards/TQBR/securities.json?iss.json=extended&securities.columns=SECID,LOTSIZE,SHORTNAME,STATUS,SECNAME,MARKETCODE,BOARDID,LISTLEVEL,PREVPRICE&iss.only=securities&iss.meta=off', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          // Add any required headers like API keys
          // 'Authorizat
          // ion': 'Bearer your-token-here'
        }
      });
      ...
      if (!Array.isArray(data)) {
        throw new Error(`Couldn't fetch data from MOEX, because its structure has changed...`);
      }   
      const securities = data[1].securities;
      // Transform the API response to match our Stock interface
      // Adjust this based on your actual API response structure
      const transformedStocks: Stock[] = securities.map((item: any) => ({
        ticker: item.SECID || '',
        name: item.SHORTNAME || '',
        currentPrice: item.PREVPRICE || 0,
        listingLevel: item.LISTLEVEL || 1,
        lotsize: item.LOTSIZE || 1
      })).filter((stock: Stock) => stock.ticker && stock.name); // Filter out invalid entries
